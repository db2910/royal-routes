        ...
        {step === 1 && (
          <motion.div key="step2" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.3 }}>
            ...
            <div className="space-y-4">
              ...
              <Input value={form.doors ?? ""} onChange={e => setForm(f => ({ ...f, doors: e.target.value }))} min={2} max={6} placeholder="e.g. 4" />
              ...
              <Input value={form.seats ?? ""} onChange={e => setForm(f => ({ ...f, seats: e.target.value }))} min={1} max={20} placeholder="e.g. 7" />
